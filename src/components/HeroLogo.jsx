import { useEffect, useRef } from "react";
import { gsap } from "gsap";
import { DrawSVGPlugin } from "gsap/DrawSVGPlugin";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(DrawSVGPlugin, ScrollTrigger);

export default function HeroLogo() {
  const svgRef = useRef();

  useEffect(() => {
    const paths = svgRef.current.querySelectorAll("path");

    gsap.set(paths, { fill: "transparent" }); // ✅ importante

    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: svgRef.current,
        start: "top 80%",
        toggleActions: "play none none none",
      },
    });

    tl.fromTo(
      paths,
      { drawSVG: "0%" },
      {
        drawSVG: "100%",
        duration: 2,
        stagger: 0.1,
        ease: "power2.out",
      }
    ).to(
      paths,
      {
        fill: "#07559f",
        duration: 1,
        ease: "power2.out",
      },
      ">-0.5"
    );
  }, []);

  return (
    <div className="w-full mx-auto md:mx-0 px-4 max-w-[320px] sm:max-w-[400px] md:max-w-[500px] lg:max-w-[600px] object-cover rounded-full text-primary">
      <svg
        ref={svgRef}
        viewBox="0 0 1100 1100"
        stroke="currentColor"
        strokeWidth="2"
        xmlns="http://www.w3.org/2000/svg"
        className="w-full h-auto"
      >
        <path
          fill="none"
          d="M492.85 75.96C279.64 94.59 94.23 275.14 76.75 489.75 45.6 742.76 243.24 982.88 497.86 1001.59c26.7 2.02 54.14 2.1 80.83 0 224.56-16.03 410.16-208.01 423.92-431.9C1020.48 283.96 778.6 48.52 492.85 75.96zm388.47 731.29c-110.7 145.52-316.9 205.5-488.12 142.31-157.65-54.72-278.31-207.78-288.42-374.72-1.03-1.93.21-4.17-.81-6.1.53-2.66-2.34-5.22-.22-7.91-.99-19.55-1.31-39.76 1.02-59.12 18.6-270.55 294.34-458.61 553.24-381.88 296.71 84.61 413.84 444.72 223.31 687.42z"
        />
        <path
          fill="none"
          d="M798.57 723.66c-30.21 24.8-66.92 41.1-106.06 44.24-147.19 18.65-268.7-92.08-254.82-241.18-1.75-2.03.58-3.92.22-5.9.95-1.6-.16-3.51.81-5.1 8.08-108.74 101.12-189.38 208.06-193.78 15.06-1.77 29.9.98 44.94 1 38.93 3.89 76.23 19.5 106.84 43.95 1.83 26.71.07 54.47.59 81.54 0 1.57 1.2 3.79-1.42 4.67-88.73-103.69-269.96-68.97-281.87 76.76-13.79 163.53 179.86 223.93 281.86 107.65 2.05.31 1.47 1.94 1.47 3.13-.58 27.56 1.21 55.8-.62 83.02z"
        />
        <path
          fill="none"
          d="M644.82 568.84c-2.71 34.14-13.38 67.26-31.02 96.62-2.83 4.76-2.69 4.58-7.7 2.34-22.61-10.13-40.7-27.67-53.95-48.35-1.4-2.06-.65-4.01.31-5.9 7.33-14.4 10.98-29.87 13.38-45.71 4.31-30.79-.39-63.26-13.35-91.81-6.04-11.13 55.15-66.66 61.9-50.13 24.98 42.77 36 94.03 30.43 142.94zM541.51 331.87c.14 1.27-1.35 1.57-2.35 2.17-7.05 4.22-14.23 8.28-21.01 12.91-21.78 14.89-40.04 33.39-55.02 55.07-1.88 2.72-3.76 3.88-7.17 3.87-55.61-.12-111.23-.15-166.84.03-4.54.02-5.45-1.24-5.4-5.53.22-21.47.21-42.95 0-64.43-.04-4.12.99-5.25 5.17-5.23 41.3.16 82.59.09 123.89.09 42.47 1.05 87.51-2 128.73 1.05zM523.44 747.45c-36.63 17.18-75.04 23.48-114.65 21.43-44.61.36-88.66-15.06-123.04-43.58-1.52-1.24-1.94-2.4-1.94-4.12.02-27.81.01-55.62.01-83.67 43.67 48.2 110.98 72.5 174.91 55.02 4.19-1.2 6.42-1.16 9.27 2.71 15.05 20.4 33.29 37.56 55.44 52.21z"
        />
      </svg>
    </div>
  );
}
